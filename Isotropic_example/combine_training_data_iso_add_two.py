...
Here, two isotropic maps are randomly added (simulating two PS populations in a single map).
...
name = "Iso_maps_combined_add_two"  # folder name for combined data
if DO_FAINT:
    name += "_faint"
if not DO_PSF:
    name += "_no_PSF"
...
if DO_FAINT:
    bins_dNdF = np.asarray([-np.infty] + list(np.logspace(-2.5, 2, 25)) + [np.infty])
else:
...
unmasked_pix = np.unique(np.asarray([settings_dict[key]["unmasked_pix"] for key in [*train_files_dict]]),
                         axis=0).squeeze()
...
if filename_out is None:
    ...
    files_train = np.asarray([filename_out + "_" + str(i).zfill(no_dig) + "_train.pickle"
                              for i in range(n_files_train)])
    files_test = np.asarray([filename_out + "_" + str(i).zfill(no_dig) + "_test.pickle"
...
for is_testing, files_dict in enumerate([train_files_dict, test_files_dict]):
    ...
    for i_file in range(start, end + 1):
        ...
        for i_temp, temp in enumerate(T_P + T_NP):
            ...
            assert np.abs(temp_map.shape[0] % 2 - 0) < 1e-4, "Need an even number of maps saved in each file!"
            # If PS map with second channel for data without PSF
            if len(temp_map.shape) == 3:
                temp_map = temp_map[:, :, i_channel_select]
            elif i_channel_select > 0:
                raise RuntimeError
            # Add up two maps?
            if ADD_TWO:
                temp_map_resh = np.reshape(temp_map, [2, -1, temp_map.shape[-1]])
                temp_map = temp_map_resh.sum(0)
            # Add to combined map
        ...
        for key in all_keys:
            ...
            for temp in T_P + T_NP:
                ...
                if key in temp_keys:
                    if ADD_TWO:
                        temp_dict[temp] = np.reshape(data_dict[temp]["info"][key], [2, -1]).T
                    else:
        ...
        if do_any_hist:
            ...
            for hist_template in hist_templates:
                ...
                n_maps_hist = n_maps_per_file // 2 if ADD_TWO else n_maps_per_file
                data_out["hists"][hist_template] = dict()

                # dNdF histogram
                if do_dNdF:
                    if "flux_arr" not in data_dict[hist_template].keys():
                        raise RuntimeError("Error! GCE PS data does NOT contain lists with flux array! Aborting...")
                    if ADD_TWO:
                        hist_input = [np.hstack([data_dict[hist_template]["flux_arr"][i].astype(np.float64),
                                                 data_dict[hist_template]["flux_arr"][i + n_maps_hist].astype(np.float64)]) \
                                        for i in range(n_maps_hist)]
                    else:
                        hist_input = [data_dict[hist_template]["flux_arr"][i].astype(np.float64)
                                      for i in range(n_maps_hist)]
                    dNdF_hist = np.asarray([np.histogram(hist_input[i], weights=hist_input[i] ** power_of_F_dNdF,
                                                             bins=bins_dNdF)[0] for i in range(n_maps_hist)])
                ...
                if do_counts_per_PS:
                    ...
                    if ADD_TWO:
                        hist_input = [np.hstack([data_dict[hist_template]["n_phot"][i].astype(np.int32),
                                                 data_dict[hist_template]["n_phot"][i + n_maps_hist].astype(np.int32)]) \
                                        for i in range(n_maps_hist)]
                    else:
                        hist_input = [data_dict[hist_template]["n_phot"][i].astype(np.int32) for i in range(n_maps_hist)]
                    counts_per_PS_hist = np.asarray([np.histogram(hist_input[i], weights=hist_input[i],
                                                         bins=bins_counts_per_PS)[0] for i in range(n_maps_hist)])
                ...
                if do_counts_per_pix:
                    ...
                    if ADD_TWO:
                        hist_input = np.reshape(data_dict[hist_template]["data"],
                                                [2, -1, *data_dict[hist_template]["data"].shape[1:]]).sum(0)
                    else:
                        hist_input = data_dict[hist_template]["data"]
                    counts_per_pix_hist = np.asarray([np.histogram(hist_input[i, :, 1], weights=hist_input[i, :, 1],
                                                         bins=bins_counts_per_pix)[0] for i in range(n_maps_hist)])
...
# i_plot = 0
# n_plot = 5
# [plt.bar(np.log10(((bins_dNdF[1:] + bins_dNdF[:-1]) / 2)[1:-1]), data_out["hists"][hist_template]["dNdF"][i][1:-1], alpha=0.8, width=0.15) \
