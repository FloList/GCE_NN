...
The GCE is a random combination of Poisson and Non-Poissonian emission, with random SCD!
"""
...
def get_gce_PS_flux_array(skew_, loc_, scale_, flux_lims_, max_total_flux, size_approx_mean=10000, flux_log=False):
...
if __name__ == '__main__':
    ...
    prior_dict["gce_12_PS"] = {"mean_exp": [-12, -9], "var_exp": 0.25, "skew_std": 3.0,
           "flux_log": False}  # NOTE: flux_lims will be defined later as [0, NN prediction]
    ...
    data_out_folder = "Best_fit_maps_random_GCE"
    data_out_file = "Maps_"
    nside = params["nside"]
    ...
    for i_chunk in range(n_chunks):
        ...
        # LENGTH: #MODELS + 1 (for smooth GCE), the order is: T_P[0], ... T_P[-1], smooth GCE, T_NP[0] (PS GCE), T_NP[1] (PS disk)
        best_fit_maps_templates = np.zeros((len(params["models"]) + 1, n_maps, n_pix_ROI))

        T_P, T_NP = params["models"][:4] + ["gce_12"], params["models"][4:]
        ...
        for i_temp, temp in enumerate(T_P):
            ...
            if i_temp < 4:
                ...
                A = T_counts_rescale_fac[i_temp]  # FIXED A, CORRESPONDING TO NN PREDICTION
                ...
                A = np.random.uniform(0, T_counts_rescale_fac[i_temp], size=(n_maps,))  # A UNIFORMLY DRAWN BETWEEN [0, NN PREDICTION]
                best_fit_maps_templates[i_temp, :, :] = np.asarray([np.random.poisson(A[i] * T) for i in range(n_maps)])
        ...
        for i_temp, temp in enumerate(T_NP):
            ...
            # For disk template: take NN best-fit prediction for the dN/dF
            if "disk" in temp:
                # relative F dN/dF -> dN/dF with correct scaling
                F_tot_template = FFs[5] * total_fermi_flux  # total flux of disk PS
            ...
            # For GCE template: take a random dN/dF drawn from the prior. This will be added to the Poissonian contribution.
            elif "gce" in temp:
                ...
                # Get the flux arrays:
                # need to rescale to account for area_template_frac
                flux_lims = [0, fluxes[4] * area_template_frac]  # flux: between 0 and the NN estimate for the GCE template
                flux_array = [get_gce_PS_flux_array(skew_draw[i], mean_draw[i], np.sqrt(var_draw[i]),
            ...
            temp_ind_in_count_fracs = 4 if "gce" in temp else 5
            print("Estimated number of counts in the Fermi map from this template:", count_fracs[temp_ind_in_count_fracs] * total_fermi_counts, "\n")
        ...
        best_fit_data = dict()
        best_fit_data["gce_hist"] = np.zeros((n_maps, len(bin_centres), len(T_NP)))  # NOTE: GCE histogram only accounts for PS contribution of course!
        ...
        GCE_count_sum_PS = best_fit_maps_templates[5].sum(1)
        GCE_count_sum_DM = best_fit_maps_templates[4].sum(1)
        GCE_count_sum_PS_sort_inds = np.argsort(GCE_count_sum_PS)
        GCE_count_sum_DM_sort_inds = np.argsort(GCE_count_sum_DM)
        rand_perm = np.random.permutation(n_maps)  # randomly permute (jointly!)
        GCE_count_sum_PS_sort_inds = GCE_count_sum_PS_sort_inds[rand_perm]
        GCE_count_sum_DM_sort_inds = GCE_count_sum_DM_sort_inds[::-1][rand_perm]  # reverse order for Poisson
        best_fit_maps_templates[4, :, :] = best_fit_maps_templates[4, GCE_count_sum_DM_sort_inds, :]
        best_fit_maps_templates[5, :, :] = best_fit_maps_templates[5, GCE_count_sum_PS_sort_inds, :]
        # Also need to sort PS flux arrays for GCE!
        flux_arrays_all[0] = np.asarray(flux_arrays_all[0])[GCE_count_sum_PS_sort_inds].tolist()

        # Compute P/(P + NP) GCE ratio
        GCE_Poiss_count_frac = best_fit_maps_templates[4, :, :].sum(1) / (best_fit_maps_templates[4, :, :].sum(1) + best_fit_maps_templates[5, :, :].sum(1))
        if i_chunk == 0 and JOB_ID == 0:
            fig_P_frac, ax_P_frac = plt.subplots(1, 1)
            ax_P_frac.hist(GCE_Poiss_count_frac)
            fig_P_frac.savefig(os.path.join(model.get_path("checkpoints"), data_out_folder, data_out_file + "_Poiss_frac_hist.pdf"), bbox_inches="tight")
            plt.close("all")
        ...
        FF_templates_combined = np.vstack([FF_templates[:4, :], (FF_templates[4, :] + FF_templates[5, :])[None], FF_templates[6:, :]])
        print("Mean FFs of simulated maps:", FF_templates.mean(1))
        print("FFs in the Fermi map:      ", FFs)
        best_fit_data["label"] = FF_templates_combined.T
        best_fit_data["label_full"] = FF_templates.T  # also store the labels before adding up DM + PS

        # Compute the Poissonian fraction within the GCE (same as GCE_Poiss_count_frac, but after dividing by exposure -> flux)
        GCE_Poiss_flux_frac = best_fit_data["label_full"][:, 4] / (best_fit_data["label_full"][:, 4] + best_fit_data["label_full"][:, 5])
        best_fit_data["gce_poiss_ff"] = np.expand_dims(GCE_Poiss_flux_frac, -1)
        ...
        #     # NOTE: GCE PS cannot be expected to agree with Fermi prediction as it's randomly drawn from the priors!
