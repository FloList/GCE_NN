...
try:
    ...
    os.environ['LD_LIBRARY_PATH'] = new_lib
######################################################
FIT = False   # Fit or load?
EXP = "Fermi_example_low_n1_w_DM"  # see below for the templates used in each experiment
RESUME = False  # resume?
...
n1_max = 30 if "low_n1" in EXP else 100
if "2_breaks" in EXP:
...
for i_task in TASK_RANGE:
    ...
    if i_task == TASK_RANGE[0]:
        mkdir_p(os.path.join("/scratch/u95/fl9575/GCE_v2/chains", TASK) if GADI else os.path.join("/home/flo/PycharmProjects/GCE/chains", TASK))
    loc_folder = os.path.join("/scratch/u95/fl9575/GCE_v2/chains", loc_task) if GADI else os.path.join("/home/flo/PycharmProjects/GCE/chains", loc_task)
    mkdir_p(loc_folder)
    ...
    if FIT:
        ...
        if i_task == TASK_RANGE[0]:
            ...
            save_loc = os.path.join("/scratch/u95/fl9575/GCE_v2/chains", TASK) if GADI \
                else os.path.join("/home/flo/PycharmProjects/GCE/DeepSphere/chains", TASK)
            mkdir_p(save_loc)
