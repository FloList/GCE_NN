"""
Difference w.r.t. constrain_Poisson_flux_fermi_with_NN_py:
...
if __name__ == '__main__':
            ...
            if i_tau < n_taus - 1:
                ...
                for i in range(len(bin_centres)):
                    ...
                    ax.fill_between(x=[bin_centres[i] - width / 2.0, bin_centres[i] + width / 2.0],
                                    ...
                                    y2=x_test_all[plot_ind, i_tau + 1].cumsum()[i], color=colors[i_tau], lw=0, alpha=0.5)
    ...
    fermi_input = np.reshape(fermi_hist_gce, [1, -1])
    ...
    ax_fermi.set_xlabel(r"Confidence level $\alpha$")
    ax_fermi.set_ylabel(r"$\tilde{\eta}_P$ [%]")
    ax_fermi.plot(all_taus, fermi_pred_Poiss_FF.flatten(), "ko", lw=0, ms=8, mec="k", mfc="white", marker="o",
                  markeredgewidth=2)
    x_ticks = np.linspace(0.0, 1.0, 6)
    ax_fermi.set_xticks(x_ticks)
    plt.tight_layout()
    fig_fermi.savefig(os.path.join(save_path, "Fermi_constraints_marginalised.pdf"), bbox_inches="tight")

    # Make a nice plot for four selected maps
    tau_inds_eval_constraint = [18, 13, 9]  # 0.95, 0.7, 0.5
    plot_inds = [57, 81, 96, 1]
    width = np.diff(bin_centres).mean()
        ...
        for i_tau in range(len(all_taus)):
            if i_tau < n_taus - 1:
                ...
                for i in range(len(bin_centres)):
                    ...
                    ax.fill_between(x=[bin_centres[i] - width / 2.0, bin_centres[i] + width / 2.0],
                                    y1=x_test_all[plot_ind, i_tau].cumsum()[i],
                                    y2=x_test_all[plot_ind, i_tau + 1].cumsum()[i], color=colors[i_tau], lw=0,
                                    alpha=0.5)
    ...
    fig.savefig(os.path.join(save_path, "selected_constraints_marginalised.pdf"), bbox_inches="tight")
