...
def healpix_weightmatrix(nside=16, nest=True, indexes=None, dtype=np.float32, new_weighting=False, do_neighbor_scaling=False):
    ...
    new_weighting : use the new weighting scheme
    do_neighbor_scaling: if True: weights are scaled by the number of neighbors contained in indexes
    """
    if not nest:
    ...
    col_index = neighbors.T.reshape((npix * 8))  # list of all the neighbours (8 * npix) (col)
    row_index = np.repeat(indexes, 8)  # list of all the indexes (row)
    ...
    if usefast:
        ...
        keep = [c in col_index_set for c in col_index]  # col indexes that are contained in indexes
    ...
    # Note: Euclidean distance is an excellent approximation of the great-circle distance for small inter-pixel distances!

    # Compute similarities / edge weights.
    if new_weighting:  # see https://github.com/deepsphere/paper-deepsphere-iclr2020/blob/51260d9169b9bff2f0d71d567c99909a17efd5e9/figures/kernel_widths.py
        if nside == 32:
            kernel_width = 0.02500
        elif nside == 64:
            kernel_width = 0.01228
        elif nside == 128:
            kernel_width = 0.00614
        elif nside == 256:
            kernel_width = 0.00307
        elif nside == 512:
            kernel_width = 0.00154
        elif nside == 1024:
            kernel_width = 0.00077
        else:  # else: determined by linear polyfit
            kernel_width = np.exp(-1.00258726 * np.log(nside) - 0.22340667)

        kernel_width_final = kernel_width ** 2 / (4 * np.log(2))
        weights = np.exp(-distances / (4 * kernel_width_final))
    else:
    ...
    # For new scheme that allows "holes": scale by number of neighbours that are contained in indexes?
    if do_neighbor_scaling and not usefast:
        frac_nb_in_indexes = np.asarray([[neighbors[v, ind] in col_index_set for ind in range(len(indexes))] for v in range(8)]).mean(0, keepdims=True)
        frac_nb_in_indexes_tiled = np.tile(frac_nb_in_indexes, [8, 1])
        frac_nb_in_indexes_tiled_rshp = frac_nb_in_indexes_tiled.T.reshape((npix * 8))
        frac_nb_in_indexes_keep = frac_nb_in_indexes_tiled_rshp[keep]
        weights /= frac_nb_in_indexes_keep
    elif usefast:
        raise NotImplementedError
    ...
    # Plot 0th pos. weight for each row
    # _, ind_for_un = np.unique(np.asarray(indexes)[row_index], return_index=True)
    # from gce_utils import masked_to_full
    # map_first_weight_per_row = masked_to_full(weights[ind_for_un], indexes, nside=nside)
    # hp.mollview(map_first_weight_per_row, nest=True)
    # plt.show()
    # fig = plt.gcf()
    # fig.savefig("First_nonzero_weight_per_row_" + str(nside) + ".pdf", bbox_inches="tight")
...
def healpix_laplacian(nside=16,
                      ...
                      use_4=False,
                      new_weighting=False,
                      do_neighbor_scaling=False):
    """Build a Healpix Laplacian."""
    if use_4:
        assert not new_weighting, "New weighting scheme not implemented for use_4!"
        W = build_matrix_4_neighboors(nside, indexes, nest=nest, dtype=dtype)
    else:
        W = healpix_weightmatrix(
            nside=nside, nest=nest, indexes=indexes, dtype=dtype, new_weighting=new_weighting,
...
def build_laplacians(nsides, indexes=None, use_4=False, new_weighting=False, do_neighbor_scaling=False):
    ...
    for i, (nside, index) in enumerate(zip(nsides, indexes)):
        ...
        if i < len(nsides) - 1:  # Last does not need a Laplacian.
            laplacian = healpix_laplacian(nside=nside, indexes=index, use_4=use_4, new_weighting=new_weighting,
                                          do_neighbor_scaling=do_neighbor_scaling)
            L.append(laplacian)
    return L, p
...
def test_learning_rates(params, ntrain, lr_min=1e-6, lr_max=1e-1, num_epochs=20, exponential=True):
    ...
    if exponential:
        ...
        params['scheduler'] = lambda step: scheduler(tf.cast(step, dtype=tf.float32))
    else:
        def scheduler(step):
