...
def plot_loss(loss_training, loss_validation, t_step, eval_frequency, logy=True):
    ...
    if logy:
        ...
        ax_step.plot(x_step, loss_training, '.-', label='training')
        ax_step.plot(x_step, loss_validation, '.-', label='validation')
    ax_time = ax_step.twiny()
    if logy:
        ax_time.semilogy(x_time, loss_training, linewidth=0)
    else:
        ax_time.plot(x_time, loss_training, linewidth=0)
