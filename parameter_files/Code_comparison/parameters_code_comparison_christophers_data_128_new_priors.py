...
def get_params(int_flag=0):
    ...
    p_gen["fermi_root"] = os.path.join("../../../Comparison_project/Data", "Christopher")  # root folder containing Fermi data
    ...
    p_data["exposure"] = "../../../Comparison_project/Data/Christopher/fermi_data_128/exp_1_bin.npy"  # one of "Fermi", "Fermi_mean", constant float, or filename of a custom exposure map
    p_data["psf"] = True  # if True: apply Fermi PSF to PS templates when generating PS maps
    p_data["psf_file"] = "../../../Comparison_project/Data/Christopher/psf_1_bin_1000000_angles.npz"
    # p_data["log_ebins"] = np.log10(np.linspace(2.0, 5.0, 11))  # log energy bins / GeV
    p_data["log_ebins"] = np.log10(np.linspace(2.0, 5.0, 2))  # log energy bins / GeV
    ...
    ###################################
    p_tt = DotDict()
    p_tt["data_name"] = "Comparison_1_christopher_new_priors"  # name of data folder for the template maps
    ...
    p_tt["n_chunk"] = int(10050)  # number of chunks to compute per job
    ...
    prior_dict["dif_silvia"] = np.asarray([1e5, 2e5])  # note that Christopher's template has a different normalization!

    # Priors for PS templates: SCDs modeled as skew normal distributions here
    prior_dict["iso_PS"] = {"mean_exp": [-12, -9], "var_exp": 1.0, "skew_std": 3.0,
                            "flux_lims": [0, 2e-07], "flux_log": False, "enforce_upper_flux": True}
    ...
    ###################################
    p_comb = DotDict()
    p_comb["data_name"] = "Comparison_1_christopher_new_priors_comb"  # name of data folder for the combined maps
    ...
    # SCD histogram settings
    p_comb["hist_templates"] = ["iso_PS"]  # list of templates for which histograms shall be saved
    ...
    p_ff["return_energies"] = False  # if True: return flux fraction per energy bin
    ...
    p_train['hist_pinball_smoothing'] = 0.0  # if > 0.0: take smoothed pinball loss (for example: 0.001)
