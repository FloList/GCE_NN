...
def get_params(int_flag=0):
    ...
    prior_dict["iso_PS"] = {"mean_exp": [-2, 1.5], "var_exp": 0.1, "skew_std": 3.0,
                            "flux_lims": [100000, 100000], "flux_log": False, "enforce_upper_flux": True}
    ...
    ###################################
    p_comb = DotDict()
    p_comb["data_name"] = "Iso_example_prd_comb"  # name of data folder for the combined maps
    p_comb["filename_base"] = "Maps"  # name basis of combined map files
    p_comb["N_val"] = 100  # number of files for the validation data set
    p_comb["N_test"] = 100  # number of files for the testing data set
    ...
    p_hist["continuous"] = True  # TODO: IMPLEMENT: estimate (pseudo)-continuous distribution rather than histogram NOTE: requires EMPL!
    ...
    p_train['num_steps'] = 25000  # number of steps to do (total number of maps shown is num_steps * batch_size)
    ...
    p_train['hist_loss'] = 'EMPL_continuous'  # loss function for the SCD histogram estimation
    ...
    p_train['hist_train_metrics'] = []  # SCD histogram metrics for tensorboard
    p["train"] = p_train
