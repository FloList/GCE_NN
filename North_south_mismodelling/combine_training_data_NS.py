"""
This script combines data per model to a combined map and calculates the flux fractions.
...
# Settings
nside = 128
input_path = '/scratch/u95/fl9575/GCE_v2/data/NS_example_maps' if HPC \
    else '/home/flo/PycharmProjects/GCE/data/NS_example_maps'
input_path += "_" + str(nside)
name = "NS_maps_combined"  # folder name for combined data
filename_out = "NS_maps"  # filename basis for combined maps
...
N_test = 500  # number of files for the test set (have NO contribution in common with training set)
...
bins_dNdF = np.asarray([-np.infty] + list(np.logspace(-1, 2, 15)) + [np.infty])
...
if SAVE_FILENAMES:
    ...
    for temp in T_P + T_NP:
        ...
        all_files = np.asarray(
            [file for file in all_files if "EXCLUDE" not in file])  # Don't include files containing "EXCLUDE"
        # Load settings dict
...
if filename_out is None:
    ...
    files_train = np.asarray(
        [filename_out + "_" + str(i).zfill(no_dig) + "_train.pickle" for i in range(n_files_train)])
            ...
            for hist_template in hist_templates:
                ...
                if do_dNdF:
                    ...
                    dNdF_hist = np.asarray([np.histogram(data_dict[hist_template]["flux_arr"][i],
                                                         weights=data_dict[hist_template]["flux_arr"][
                                                                     i] ** power_of_F_dNdF,
                                                         bins=bins_dNdF)[0] for i in range(n_maps_per_file)])
                    dNdF_hist_sum = dNdF_hist.sum(1)

                    data_out["hists"][hist_template]["dNdF"] = dNdF_hist / np.expand_dims(
                        get_denominator(dNdF_hist_sum), -1)

                # counts per PS histogram
                if do_counts_per_PS:
                    if "n_phot" not in data_dict[hist_template].keys():
                    ...
                    counts_per_PS_hist = np.asarray([np.histogram(data_dict[hist_template]["n_phot"][i],
                                                                  ...
                                                                  bins=bins_counts_per_PS)[0] for i in
                                                     range(n_maps_per_file)])
                    ...
                    data_out["hists"][hist_template]["counts_per_PS"] = counts_per_PS_hist / np.expand_dims(
                        get_denominator(counts_per_PS_hist_sum), -1)

                # counts per pixel histogram
                if do_counts_per_pix:
                    if len(data_dict[hist_template]["data"].shape) != 3:
                        raise RuntimeError(
                            "Error! Data does NOT contain second channel with map before PSF application! Aborting...")
                    counts_per_pix_hist = np.asarray([np.histogram(data_dict[hist_template]["data"][i, :, 1],
                                                                   weights=data_dict[hist_template]["data"][i, :, 1],
                                                                   bins=bins_counts_per_pix)[0] for i in
                                                      range(n_maps_per_file)])
                    counts_per_pix_hist_sum = counts_per_pix_hist.sum(1)
                    data_out["hists"][hist_template]["counts_per_pix"] = counts_per_pix_hist / np.expand_dims(
                        get_denominator(counts_per_pix_hist_sum), -1)
...
# [plt.bar(np.log(((bins_dNdF[1:] + bins_dNdF[:-1]) / 2)[1:-1]), data_out["hists"][hist_template]["dNdF"][i][1:-1]) for i in range(0, 5)]
